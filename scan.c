...
mt76_scan_set_phy(struct mt76_dev *dev, struct ieee80211_channel *chan,
		  bool reset_chan)
{
	struct mt76_phy *phy, *prev_phy;

	prev_phy = dev->scan.phy;
	if (!dev->phy.hw->wiphy->n_radio)
		return prev_phy;
	...
	if (reset_chan)
		mt76_set_channel(prev_phy, &prev_phy->main_chandef, false);

	clear_bit(MT76_SCANNING, &prev_phy->state);
...
void mt76_scan_work(struct work_struct *work)
{
	...
	phy = mt76_scan_set_phy(dev, dev->scan.chan, reset_chan);
	if (!phy)
		goto out;
